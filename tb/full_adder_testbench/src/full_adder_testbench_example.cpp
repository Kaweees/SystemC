...
int sc_main(int argc, char* argv[]) {
  ...
  sc_trace_file* tf = sc_create_vcd_trace_file("full_adder_traces");
  if (tf) {
    sc_trace(tf, tb.clk_sig, "clk");
    sc_trace(tf, tb.a_sig, "a");
    sc_trace(tf, tb.b_sig, "b");
    sc_trace(tf, tb.cin_sig, "cin");
    sc_trace(tf, tb.sum_sig, "sum");
    sc_trace(tf, tb.cout_sig, "cout");
    sc_trace(tf, tb.test_complete_sig, "test_complete");
  }
